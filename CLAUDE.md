...
- **PPSSPP runs on the THIN wrapper, on Vulkan** (no force-route since `2f09fd7802`, 2026-05-28). On GLES, PPSSPP spawns its own render thread (`useEmuThread = GPUCORE_GLES`) that never has a current EAGL context, so GL can't work on thin; `GET_PREFERRED_HW_RENDER` returns Vulkan for PSP and GL is rejected (`e66b2a28a4`, `79a091c4b2`), with `context_reset` deferred until after `retro_load_game`. Known unfixed hazards in `PVThinLibretroFrontend.mm`: a failed deferred Vulkan setup still reports a successful load, and a failed `vkQueueSubmit` can leave a fence that `thin_vulkan_wait_sync_index` waits on forever while holding `_vulkanQueueLock`.
