...
//    func supportedCores(forGame game: PVGame) -> [PVCore] {
//        return game.system.cores
//            .filter { !$0.disabled || PVSettingsModel.shared.debugOptions.unsupportedCores }
//            .map { aCore -> Button in
//                Button("\(aCore.title)", role: .cancel) {
//                    rootDelegate?.root_load(game, sender: self, core: aCore, saveState: nil)
//                }
//            }
//    }
    ...
    @State private var showingMultipleCoresAlert = false
    @State private var showingNoCoresAlert = false
        ...
        ScrollView {
            ...
            if viewModel.viewGamesAsGrid {
                LazyVGrid(columns: horizontalSizeClass == .compact ?
                          columnsCompact : columnsRegular, spacing: 20) {
                ...
                LazyVStack {
                    ForEach(filteredAndSortedGames(), id: \.self) { game in
                        GameItemView(game: game, viewType: .row) {
                            if game.system.cores.count == 0 {
                                showingNoCoresAlert = true
                            } else if game.system.cores.count == 1 {
                                rootDelegate?.root_load(game, sender: self, core: nil, saveState: nil)
                            } else {
                                showingMultipleCoresAlert = true
                            }
                        }
                        .alert("No cores found.", isPresented: $showingNoCoresAlert) {
                            Button("OK", role: .cancel) { }
                          }
                        .alert("Multiple cores found.", isPresented: $showingMultipleCoresAlert) {
//                            supportedCores(forGame: game).map {
//                                Button("\($0.title)", role: .done) {
//                                    rootDelegate?.root_load(game, sender: self, core: $0, saveState: nil)
//                                }
//                            }
                          }
                        .contextMenu { GameContextMenu(game: game, rootDelegate: rootDelegate) }
...
struct BiosRowView: SwiftUI.View {
    ...
    @ObservedRealmObject var bios: PVBIOS
    ...
    var body: some SwiftUI.View {
        HStack(alignment: .center, spacing: 0) {
            ...
            HStack(alignment: .center, spacing: 4) {
                ...
                case .missing:
                    Text("Missing \(self.bios.optional ? " (Optional)" : "")")
                        .font(.system(size: 12))
                        .foregroundColor(self.bios.optional ? Color.yellow : Color.red)
