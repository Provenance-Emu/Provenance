...
class PVSettingsViewController: UITableViewController, SFSafariViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        opacitySlider.value = settings.controllerOpacity()
...
#endif
        let revisionString = Bundle.main.infoDictionary?["Revision"] as? String
        let color: UIColor? = UIColor(white: 0.0, alpha: 0.1)
        if revisionString?.count > 0 {
    ...
    @IBAction func toggleFPSCount(_ sender: Any) {
        PVSettingsModel.sharedInstance().setShowFPSCount(fpsCountSwitch.isOn())
    }

    @IBAction func toggleAutoSave(_ sender: Any) {
        PVSettingsModel.sharedInstance().setAutoSave(autoSaveSwitch.isOn())
    }

    @IBAction func toggleAutoLoadAutoSaves(_ sender: Any) {
        PVSettingsModel.sharedInstance().setAutoLoadAutoSaves(autoLoadSwitch.isOn())
    }

    @IBAction func controllerOpacityChanged(_ sender: Any) {
        opacitySlider.value = floor(opacitySlider.value / 0.05) * 0.05
        opacityValueLabel.text = String(format: "%.0f%%", opacitySlider.value * 100)
        PVSettingsModel.sharedInstance().setControllerOpacity(opacitySlider.value)
    }

    @IBAction func toggleAutoLock(_ sender: Any) {
        PVSettingsModel.sharedInstance().setDisableAutoLock(autoLockSwitch.isOn())
    }

    @IBAction func toggleVibration(_ sender: Any) {
        PVSettingsModel.sharedInstance().setButtonVibration(vibrateSwitch.isOn())
    }

    @IBAction func toggleSmoothing(_ sender: Any) {
        PVSettingsModel.sharedInstance().setImageSmoothing(isImageSmoothing.isOn())
    }

    @IBAction func toggleCRTFilter(_ sender: Any) {
        PVSettingsModel.sharedInstance().setCrtFilterEnabled(crtFilterSwitch.isOn())
    }

    @IBAction func volumeChanged(_ sender: Any) {
        PVSettingsModel.sharedInstance().setVolume(volumeSlider.value)
        volumeValueLabel.text = String(format: "%.0f%%", volumeSlider.value * 100)
    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == 3 && indexPath.row == 0 {
            ...
            alert.addAction(UIAlertAction(title: "Yes", style: .default, handler: {(_ action: UIAlertAction) -> Void in
                NotificationCenter.default.post(name: NSNotification.pvRefreshLibraryNotification, object: nil)
            }))
        ...
        navigationItem.setRightBarButton(UIBarButtonItem(barButtonSystemItem: .done, target: self, action: Selector("done:")), animated: false)
    }
}