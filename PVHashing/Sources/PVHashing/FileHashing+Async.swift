...
public enum MD5HashingEvent: Sendable {
    ...
    /// Final event when hashing is complete, carrying the hex MD5 string.
...
/// `.completed` event carrying the lowercase hex MD5 string, followed by
/// stream termination. If the task is cancelled the stream finishes with a
/// `CancellationError`.
///
/// - Parameters:
///   - fileURL: The URL of the file to hash.
    ...
    AsyncThrowingStream { continuation in
        let task = Task.detached(priority: .utility) {
            do {
                ...
                let totalBytes = max(0, fileSize - Int64(offset))
...
/// Core blocking MD5 computation used by both the sync wrapper and `calculateMD5Async`.
