...
- (BOOL)filterShaderEnabled {
    NSString *value = PVSettingsModel.shared.shared.metalFilter.lowercaseString;
...
- (void)viewDidLoad
{
    ...
    if (self.emulatorCore.rendersToOpenGL)
    {
        ...
        self.glContext.multiThreaded = PVSettingsModel.shared.shared.videoOptions.multiThreadedGL;
...
//     CVDisplayLinkStart(_caDisplayLink);
    ...
    if (PVSettingsModel.shared.shared.nativeScaleEnabled) {
        ...
        if(PVSettingsModel.shared.shared.videoOptions.multiSampling) {
    ...
    NSString *metalFilter = PVSettingsModel.shared.shared.metalFilter;
...
- (void)viewDidLayoutSubviews
{
    ...
    if (!CGRectIsEmpty([self.emulatorCore screenRect]))
    {
        ...
        if (parentSize.width > parentSize.height) {
            if (PVSettingsModel.shared.shared.integerScaleEnabled) { height = (floor(parentSize.height/aspectSize.height)) * aspectSize.height;}
            else
            ...
            if (PVSettingsModel.shared.shared.integerScaleEnabled) { width = (floor(parentSize.width/aspectSize.width)) * aspectSize.width;}
            else
            {width = parentSize.width;}
            height = roundf(width / ratio);
