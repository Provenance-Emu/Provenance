...
public class RetroProgressHUD: UIView {
    ...
    private var backgroundOverlayColor: UIColor {
        palette.dark ? UIColor.black.withAlphaComponent(0.7) : UIColor.white.withAlphaComponent(0.9)
    }

    private var containerBackgroundColor: UIColor {
        palette.settingsCellBackground ?? palette.gameLibraryBackground
    }

    private var titleTextColor: UIColor {
        palette.settingsCellText ?? palette.gameLibraryText
    }

    private var containerBorderColor: UIColor {
        palette.defaultTintColor
    }

    private var gridLineColor: UIColor {
        palette.defaultTintColor.withAlphaComponent(palette.dark ? 0.2 : 0.1)
    }
    ...
    private func updateThemeColors() {
        ...
        // Update grid pattern
        if let gridPattern = containerView.subviews.first(where: { $0.tag == 999 }) {
            gridPattern.removeFromSuperview()
        }
        let newGridPattern = createGridPattern()
        newGridPattern.tag = 999
        newGridPattern.alpha = palette.dark ? 0.3 : 0.2
        containerView.insertSubview(newGridPattern, at: 0)
        NSLayoutConstraint.activate([
            newGridPattern.topAnchor.constraint(equalTo: containerView.topAnchor),
            newGridPattern.leadingAnchor.constraint(equalTo: containerView.leadingAnchor),
            newGridPattern.trailingAnchor.constraint(equalTo: containerView.trailingAnchor),
            newGridPattern.bottomAnchor.constraint(equalTo: containerView.bottomAnchor)
        ])

        // Update glow effect
        addGlowEffect(to: containerView)

        // Update loading indicator and progress bar
        loadingIndicator.updateTheme()
        progressView?.updateTheme()
    }
    ...
    private func setupViews() {
        ...
        gridPattern.tag = 999
        gridPattern.alpha = palette.dark ? 0.3 : 0.2
    ...
    private func createGridPattern() -> UIView {
        ...
        for i in 0...10 {
            let lineView = UIView()
            lineView.backgroundColor = gridLineColor
    ...
    private func addGlowEffect(to view: UIView) {
        ...
        view.layer.shadowOpacity = palette.dark ? 0.8 : 0.5
...
class RetroLoadingIndicator: UIView {
    ...
    private var secondaryColor: UIColor {
        palette.settingsHeaderText ?? palette.defaultTintColor
    }
    ...
    private func setupLayers() {
        ...
        glowLayer.strokeColor = secondaryColor.cgColor
...
class CustomRetroProgressBar: UIView {
    ...
    private var trackColor: UIColor {
        palette.dark ? UIColor.darkGray : UIColor.lightGray
    }
    ...
    private func setupLayers() {
        ...
        glowLayer.shadowOpacity = palette.dark ? 0.8 : 0.5
