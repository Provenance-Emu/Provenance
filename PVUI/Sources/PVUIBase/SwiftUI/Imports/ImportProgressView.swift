...
public struct ImportProgressView: View {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if let progress = viewModel.initialSyncProgress {
                        VStack(alignment: .leading, spacing: 2) {
                            // ROMs progress
                            if progress.romsTotal > 0 {
                                Text("ROMs: \(progress.romsCompleted)/\(progress.romsTotal)")
                                    .font(.caption)
                                    .foregroundColor(.retroBlue)
                            }

                            // Save states progress
                            if progress.saveStatesTotal > 0 {
                                Text("Save States: \(progress.saveStatesCompleted)/\(progress.saveStatesTotal)")
                                    .font(.caption)
                                    .foregroundColor(.retroPurple)
                            }

                            // BIOS progress
                            if progress.biosTotal > 0 {
                                Text("BIOS: \(progress.biosCompleted)/\(progress.biosTotal)")
                                    .font(.caption)
                                    .foregroundColor(.retroPink)
                            }
                            
                            // Battery States progress
                            if progress.batteryStatesTotal > 0 {
                                Text("Battery States: \(progress.batteryStatesCompleted)/\(progress.batteryStatesTotal)")
                                    .font(.caption)
                                    .foregroundColor(.retroBlue)
                            }
                            
                            // Screenshots progress
                            if progress.screenshotsTotal > 0 {
                                Text("Screenshots: \(progress.screenshotsCompleted)/\(progress.screenshotsTotal)")
                                    .font(.caption)
                                    .foregroundColor(.retroPurple)
                            }
                            
                            // DeltaSkins progress
                            if progress.deltaSkinsTotal > 0 {
                                Text("Delta Skins: \(progress.deltaSkinsCompleted)/\(progress.deltaSkinsTotal)")
                                    .font(.caption)
                                    .foregroundColor(.retroPink)
                            }
    ...
    class ViewModel: ObservableObject {
        ...
        @Published var initialSyncProgress: InitialSyncProgress?
        ...
        private func setupCloudKitSubscriptions() {
            ...
            // Subscribe to initial sync progress updates
            Task {
                do {
