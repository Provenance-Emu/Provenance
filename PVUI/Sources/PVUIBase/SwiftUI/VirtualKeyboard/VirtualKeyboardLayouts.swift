...
public enum VirtualKeyboardLayouts {
    ...
    public static let atariSTLayout: [[VirtualKey]] = [
        ...
        // Row 7: Numeric keypad
        // Note: The Atari ST keypad has ( and ) keys at the positions occupied by - and + on a
        // standard PC keypad.  The labels show the Atari ST keycap; the keyCodes use the
        // physical PC keypad equivalents (.keypadHyphen for ( and .keypadPlus for )) so that
        // Hatari receives the correct scancode for those positions.
        [
            .init(label: "7",  keyCode: .keypad7),
            .init(label: "8",  keyCode: .keypad8),
            .init(label: "9",  keyCode: .keypad9),
