...
private extension JSButton {
    var buttonTag: PVMSXButton {
        get {
            guard let mapped = PVMSXButton(rawValue: tag) else {
                ...
                ELOG("PVMSXController: unknown JSButton tag \(tag) — defaulting to .up (likely an auxiliary button, not an emulator input)")
                return .up
            }
            return mapped
        }
