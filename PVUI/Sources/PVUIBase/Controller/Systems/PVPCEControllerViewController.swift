...
private extension JSButton {
    var buttonTag: PVPCEButton {
        get {
            guard let mapped = PVPCEButton(rawValue: tag) else {
                ...
                ELOG("PC EngineController: unknown JSButton tag \(tag) — defaulting to .up (likely an auxiliary button, not an emulator input)")
                return .up
            }
            return mapped
        }
...
final class PVPCEControllerViewController: PVControllerViewController<PVPCESystemResponderClient> {
    ...
    override func didReceiveHardwareSwitchInput(buttonId: String, player: Int) {
        let button = PVPCEButton(buttonId)
