...
private extension JSButton {
    var buttonTag: PVDOSButton {
        get {
            guard let mapped = PVDOSButton(rawValue: tag) else {
                ...
                ELOG("PVDOSController: unknown JSButton tag \(tag) — defaulting to .up (likely an auxiliary button, not an emulator input)")
                return .up
            }
            return mapped
        }
