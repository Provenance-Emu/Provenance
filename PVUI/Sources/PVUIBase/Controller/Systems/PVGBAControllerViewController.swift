...
private extension JSButton {
    var buttonTag: PVGBAButton {
        get {
            guard let mapped = PVGBAButton(rawValue: tag) else {
                ...
                ELOG("PVGBAController: unknown JSButton tag \(tag) — defaulting to .up (likely an auxiliary button, not an emulator input)")
                return .up
            }
            return mapped
        }
