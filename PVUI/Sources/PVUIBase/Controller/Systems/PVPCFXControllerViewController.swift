...
private extension JSButton {
    var buttonTag: PVPCFXButton {
        get {
            guard let mapped = PVPCFXButton(rawValue: tag) else {
                ...
                ELOG("PC-FXController: unknown JSButton tag \(tag) — defaulting to .up (likely an auxiliary button, not an emulator input)")
                return .up
            }
            return mapped
        }
