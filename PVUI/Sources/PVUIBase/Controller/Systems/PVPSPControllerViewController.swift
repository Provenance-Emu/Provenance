...
private extension JSButton {
    var buttonTag: PVPSPButton {
        get {
            guard let mapped = PVPSPButton(rawValue: tag) else {
                ...
                ELOG("PVPSPController: unknown JSButton tag \(tag) — defaulting to .up (likely an auxiliary button, not an emulator input)")
                return .up
            }
            return mapped
        }
