...
private extension JSButton {
    var buttonTag: PV5200Button {
        get {
            guard let mapped = PV5200Button(rawValue: tag) else {
                ...
                ELOG("Atari 5200Controller: unknown JSButton tag \(tag) — defaulting to .up (likely an auxiliary button, not an emulator input)")
                return .up
            }
            return mapped
        }
...
final class PVAtari5200ControllerViewController: PVControllerViewController<PV5200SystemResponderClient> {
    ...
    override func didReceiveHardwareSwitchInput(buttonId: String, player: Int) {
        let button = PV5200Button(buttonId)
