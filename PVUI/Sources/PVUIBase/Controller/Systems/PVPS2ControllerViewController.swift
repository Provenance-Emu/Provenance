...
private extension JSButton {
    var buttonTag: PVPS2Button {
        get {
            guard let mapped = PVPS2Button(rawValue: tag) else {
                ...
                ELOG("PS2Controller: unknown JSButton tag \(tag) — defaulting to .up (likely an auxiliary button, not an emulator input)")
                return .up
            }
            return mapped
        }
