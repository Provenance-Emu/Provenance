...
private extension JSButton {
    var buttonTag: PV3DSButton {
        get {
            guard let mapped = PV3DSButton(rawValue: tag) else {
                ...
                ELOG("3DSController: unknown JSButton tag \(tag) — defaulting to .up (likely an auxiliary button, not an emulator input)")
                return .up
            }
            return mapped
        }
