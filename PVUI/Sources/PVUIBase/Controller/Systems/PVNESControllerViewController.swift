...
private extension JSButton {
    var buttonTag: PVNESButton {
        get {
            guard let mapped = PVNESButton(rawValue: tag) else {
                ...
                ELOG("PVNESController: unknown JSButton tag \(tag) — defaulting to .up (likely an auxiliary button, not an emulator input)")
                return .up
            }
            return mapped
        }
