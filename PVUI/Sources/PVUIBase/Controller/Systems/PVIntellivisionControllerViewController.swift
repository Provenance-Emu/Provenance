...
private extension JSButton {
    var buttonTag: PVIntellivisionButton {
        get {
            guard let mapped = PVIntellivisionButton(rawValue: tag) else {
                ...
                ELOG("PVIntellivisionController: unknown JSButton tag \(tag) — defaulting to .up (likely an auxiliary button, not an emulator input)")
                return .up
            }
            return mapped
        }
