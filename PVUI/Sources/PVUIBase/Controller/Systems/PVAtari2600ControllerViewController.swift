...
private extension JSButton {
    var buttonTag: PV2600Button {
        get {
            guard let mapped = PV2600Button(rawValue: tag) else {
                ...
                ELOG("Atari 2600Controller: unknown JSButton tag \(tag) — defaulting to .up (likely an auxiliary button, not an emulator input)")
                return .up
            }
            return mapped
        }
