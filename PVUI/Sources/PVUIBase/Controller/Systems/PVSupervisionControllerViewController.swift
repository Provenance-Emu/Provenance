...
private extension JSButton {
    var buttonTag: PVSupervisionButton {
        get {
            guard let mapped = PVSupervisionButton(rawValue: tag) else {
                ...
                ELOG("PVSupervisionController: unknown JSButton tag \(tag) — defaulting to .up (likely an auxiliary button, not an emulator input)")
                return .up
            }
            return mapped
        }
