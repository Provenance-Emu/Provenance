...
//  v09062020a
//
//  Updated for Provenance Metal 250302
//
...
float4 megaTronTone(thread const float& contrast, thread const float& saturation, thread const float& thin, thread float& mask, constant MegaTronUniforms& params)
{
    if (params.MASK == 0.0)
    {
        mask = 1.0;
...
static inline __attribute__((always_inline))
float FromSrgbFunc(thread const float& c, constant MegaTronUniforms& params)
{
    float _69;
    if (c <= 0.04045)
    {
...
static inline __attribute__((always_inline))
float3 FromSrgb(thread const float3& c, constant MegaTronUniforms& params)
{
    float param = c.x;
    float param_1 = c.y;
    float param_2 = c.z;
...
float3 megaTronFilter(thread const float2& ipos,
                  ...
                  constant MegaTronUniforms& params,
...
megaTron(Outputs in [[stage_in]],
       constant MegaTronUniforms& params [[buffer(0)]],
