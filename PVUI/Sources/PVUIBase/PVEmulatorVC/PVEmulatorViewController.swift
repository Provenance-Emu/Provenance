...
final class PVEmulatorViewController: PVEmulatorViewControllerRootClass, PVAudioDelegate, PVSaveStatesViewControllerDelegate {
    ...
    var batterySavesPath: URL { get async {return await PVEmulatorConfiguration.batterySavesPath(forGame: game) }}
    var BIOSPath: URL { get async { return await PVEmulatorConfiguration.biosPath(forGame: game) } }
    ...
    private func initCore() async {
        core.audioDelegate = self
        core.saveStatesPath = await saveStatePath.path
        core.batterySavesPath = await batterySavesPath.path
        core.BIOSPath = await BIOSPath.path
    ...
    private func handleArchives(atPath romPathMaybe: URL?) async -> URL? {
        ...
        if core.extractArchive, let filePath = romPathMaybe {
            if (filePath.pathExtension.caseInsensitiveCompare("zip") == .orderedSame) {
                ...
                Task {
                    let savePath = await self.batterySavesPath.standardizedFileURL
    ...
    private func createEmulator() async {
        ...
        let m3uFile: URL? = await PVEmulatorConfiguration.m3uFile(forGame: game)
        var romPathMaybe: URL? = UserDefaults.standard.url(forKey: game.romPath) ?? m3uFile
        if romPathMaybe == nil {
            romPathMaybe = await game.file.url
        }
        ...
        await convertOldSaveStatesToNewIfNeeded()
    ...
    @objc func takeScreenshot() async {
        if let screenshot = captureScreenshot() {
            ...
            if let pngData = screenshot.pngData() {
                ...
                let imageURL = await PVEmulatorConfiguration.screenshotsPath(forGame: self.game).appendingPathComponent(fileName, isDirectory: false)
                do {
