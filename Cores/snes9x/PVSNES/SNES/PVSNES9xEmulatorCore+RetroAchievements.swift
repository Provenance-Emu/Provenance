...
//  PVSNES
//
//  Conformance of PVSNES9xEmulatorCore to CoreRetroAchievements.
//
//  ## Current status: protocol conformance stub
//
//  snes9x does not include rcheevos.  Full integration requires:
//  1. Add a shared PVRcheevos SPM target (wrapping rcheevos C library) as a
//     dependency of PVSNES in Package.swift.
//  2. After `loadFileAtPath:`, hash the ROM and call rc_client_load_game().
//  3. At end of each frame, call rc_client_do_frame() — hook into
//     PVSNESEmulatorCoreBridge's executeFrame path.
//  4. Expose SNES WRAM (0x7E0000–0x7FFFFF, 128 KiB) in achievementMemoryRegions().
...
extension PVSNES9xEmulatorCore: CoreRetroAchievements {
    ...
    public func achievementMemoryRegions() -> [AchievementMemoryRegion] {
        // TODO: expose SNES WRAM (128 KiB) at address 0x7E0000 once wired.
        return []
    }
