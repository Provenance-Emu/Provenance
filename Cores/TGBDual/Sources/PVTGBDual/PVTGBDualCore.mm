...
- (BOOL)loadFileAtPath:(NSString *)path error:(NSError **)error {
    ...
    if(retro_load_game(&info)) {
        ...
        self._videoWidth = info.geometry.max_width;
        self._videoHeight = info.geometry.max_height;
        self._videoBuffer = (uint16_t*)malloc(self._videoWidth * self._videoHeight * 2 * 2);
 ...
 - (void)startEmulation {
     if(self.isRunning == false) {
         self.emulationHasRun = true;
         [super startEmulation];
     }
}

- (void)stopEmulation {
    if (self.emulationHasRun) {
        if([self.batterySavesPath length]) {
