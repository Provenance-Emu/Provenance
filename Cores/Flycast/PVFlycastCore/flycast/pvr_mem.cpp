...
// Local Changes: Mem Assignment Check
#include "pvr_mem.h"
...
void DYNACALL pvr_write32p(u32 addr, T data)
{
	...
	if (pvr_map32(addr) >= 0 &&  pvr_map32(addr) <  VRAM_SIZE && &data && (T *)&vram[pvr_map32(addr)] && ((data)  || ((*(T *)&vram[pvr_map32(addr)] )))) {
         addrspace::unprotectVram(0, VRAM_SIZE);
         /*std::memcpy((void*)(&vram[pvr_map32(addr)]),
                     (void*)&data,
                     sizeof(T));*/
...
void DYNACALL pvr_write_area4(u32 addr, T data)
{
	...
	if (access32)
        ...
        if (addr & VRAM_MASK >= 0 &&  addr & VRAM_MASK <  VRAM_SIZE && &data && (T*)&vram[addr & VRAM_MASK] && ((data)  || ((*(T*)&vram[addr & VRAM_MASK])))) {
                 addrspace::unprotectVram(0, VRAM_SIZE);
                 /*std::memcpy((void*)(&vram[addr & VRAM_MASK]),
                             (void*)&data,
                             sizeof(T)); */
                 *(T*)&vram[addr & VRAM_MASK] = data;
         }
