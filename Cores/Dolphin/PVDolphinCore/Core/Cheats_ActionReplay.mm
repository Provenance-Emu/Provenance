...
static void VLogInfo(std::string_view format, fmt::format_args args)
{
  ...
  // Upstream renamed MAX_LOGLEVEL to MAX_EFFECTIVE_LOGLEVEL and made the check
  // `if constexpr`; mirror ActionReplay.cpp so the log level compiles out again.
...
static bool Subtype_AddCode(const Core::CPUThreadGuard& guard, const ARAddr& addr, const u32 data)
{
  ...
  case DATATYPE_8BIT:
    ...
    LogInfo("Wrote {:02x} to address {:08x}", PowerPC::MMU::HostRead<u8>(guard, new_addr), new_addr);
    LogInfo("--------");
    break;
  ...
  case DATATYPE_16BIT:
    ...
    LogInfo("Wrote {:04x} to address {:08x}", PowerPC::MMU::HostRead<u16>(guard, new_addr), new_addr);
    LogInfo("--------");
    break;
  ...
  case DATATYPE_32BIT:
    ...
    LogInfo("Wrote {:08x} to address {:08x}", PowerPC::MMU::HostRead<u32>(guard, new_addr), new_addr);
    LogInfo("--------");
    break;
  ...
  if ((data & 0xFF0000) == 0)
  {
    ...
    {  // Memory Copy With Pointers Support
      ...
      for (int i = 0; i < num_bytes; ++i)
      {
        ...
        LogInfo("Wrote {:08x} to address {:08x}", PowerPC::MMU::HostRead<u8>(guard, ptr_src + i), ptr_dest + i);
      }
      LogInfo("--------");
    }
    else
...
static bool ConditionalCode(const Core::CPUThreadGuard& guard, const ARAddr& addr, const u32 data, int* const pSkipCount)
{
  ...
  case DATATYPE_16BIT:
    result = CompareValues(PowerPC::MMU::HostRead<u16>(guard, new_addr), (data & 0xFFFF), addr.type);
    break;
