...
-(instancetype) init {
    if (self = [super init]) {
        ...
        InitializeLogging();
#else
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"PVAzaharCore.Enable Logging"]) {
            InitializeLogging();
        }
#endif
...
-(void) setShaderOption {
    unsigned int shaderType = [[NSNumber numberWithInteger:[[NSUserDefaults standardUserDefaults] integerForKey:@"PVAzaharCore.Shader Acceleration / Graphic Accuracy"]] unsignedIntValue];
...
-(void) setOptions:(bool)resetButtons {
    Config{};
    Settings::values.layout_option.SetValue((Settings::LayoutOption)[[NSNumber numberWithInteger:[[NSUserDefaults standardUserDefaults] integerForKey:@"PVAzaharCore.Portrait Layout"]] unsignedIntValue]);
    Settings::values.resolution_factor.SetValue([[NSNumber numberWithInteger:[[NSUserDefaults standardUserDefaults] integerForKey:@"PVAzaharCore.Resolution Upscaling"]] unsignedIntValue]);
    Settings::values.async_shader_compilation.SetValue([[NSUserDefaults standardUserDefaults] boolForKey:@"PVAzaharCore.Enable Async Shader Compilation"]);
    Settings::values.async_presentation.SetValue([[NSUserDefaults standardUserDefaults] boolForKey:@"PVAzaharCore.Enable Async Presentation"]);
    unsigned int shaderType = [[NSNumber numberWithInteger:[[NSUserDefaults standardUserDefaults] integerForKey:@"PVAzaharCore.Shader Acceleration / Graphic Accuracy"]] unsignedIntValue];
    Settings::values.use_hw_shader.SetValue(shaderType >= 2);
    Settings::values.shader_type.SetValue(shaderType);

    Settings::values.use_cpu_jit.SetValue([[NSUserDefaults standardUserDefaults] boolForKey:@"PVAzaharCore.Enable Just in Time"]);
    Settings::values.cpu_clock_percentage.SetValue([[NSNumber numberWithInteger:[[NSUserDefaults standardUserDefaults] integerForKey:@"PVAzaharCore.CPU Clock Speed"]] unsignedIntValue]);
    Settings::values.is_new_3ds.SetValue([[NSUserDefaults standardUserDefaults] boolForKey:@"PVAzaharCore.Enable New 3DS"]);
    

    BOOL enabledLogging = [[NSUserDefaults standardUserDefaults] boolForKey:@"PVAzaharCore.Enable Logging"];
    if (enabledLogging) {
#if DEBUG
    ...
    Settings::values.upright_screen.SetValue([[NSUserDefaults standardUserDefaults] boolForKey:@"PVAzaharCore.Upright Screen"]);

    Settings::values.render_3d.SetValue((Settings::StereoRenderOption)[[NSNumber numberWithInteger:[[NSUserDefaults standardUserDefaults] integerForKey:@"PVAzaharCore.3D Stereo Render"]] unsignedIntValue]);
    Settings::values.factor_3d.SetValue([[NSNumber numberWithInteger:[[NSUserDefaults standardUserDefaults] integerForKey:@"PVAzaharCore.3D Factor"]] unsignedIntValue]);

    Settings::values.dump_textures.SetValue([[NSUserDefaults standardUserDefaults] boolForKey:@"dump_textures"]);
    Settings::values.custom_textures.SetValue([[NSUserDefaults standardUserDefaults] boolForKey:@"PVAzaharCore.Use Custom Textures"]);
    Settings::values.preload_textures.SetValue([[NSUserDefaults standardUserDefaults] boolForKey:@"PVAzaharCore.Preload Textures"]);
    ...
    // Stretch Audio
    shouldStretchAudio=[[NSUserDefaults standardUserDefaults] boolForKey:@"PVAzaharCore.Stretch Audio"];
    Settings::values.enable_audio_stretching.SetValue(shouldStretchAudio);
    
    // Realtime audio
    BOOL enable_realtime_audio = [[NSUserDefaults standardUserDefaults] boolForKey:@"PVAzaharCore.Realtime Audio"];
    Settings::values.enable_realtime_audio.SetValue(enable_realtime_audio);

    // Volume
    int volume = [[NSNumber numberWithInteger:[[NSUserDefaults standardUserDefaults] integerForKey:@"PVAzaharCore.Audio Volume"]] unsignedIntValue];
    Settings::values.volume.SetValue((float)volume / 100.0);
    
    // Microphone
    int inputType = [[NSNumber numberWithInteger:[[NSUserDefaults standardUserDefaults] integerForKey:@"PVAzaharCore.Microphone Input"]] unsignedIntValue];
    Settings::values.input_type.SetValue((AudioCore::InputType) inputType);

    // Region
    int retionType = [[NSNumber numberWithInteger:[[NSUserDefaults standardUserDefaults] integerForKey:@"PVAzaharCore.System Region"]] intValue];
    Settings::values.region_value.SetValue(retionType);
    
    if (resetButtons)
        [CitraWrapper.sharedInstance setButtons];
    for (const auto& service_module : Service::service_module_map) {
        Settings::values.lle_modules.emplace(service_module.name, ![[NSUserDefaults standardUserDefaults] boolForKey:@"PVAzaharCore.Enable High Level Emulation"]);
    }
