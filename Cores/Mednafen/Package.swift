...
let CSETTINGS: [CSetting] =
  [
    ...
    .define(
      "HAVE_FORK", to: "1", .when(platforms: [.iOS, .macOS, .watchOS, .macCatalyst, .visionOS])),
...
func mednafenTarget(
  name: String, dependencies: [String] = [], path: String, sources: [String]? = nil
...
let targets: [Target] = [
  ...
  // Vendored Zstandard (decompression only) for iOS/macOS builds
  .target(
    name: "zstd",
    ...
    cSettings: [
      ...
      // Expose static-linking only definitions required by some internal headers (e.g. allocations.h/pool.h)
      .define("ZSTD_STATIC_LINKING_ONLY", to: "1")
    ]
  ),
  // Vendored LZMA (7-Zip SDK) required by libchdr for some CHD variants
  .target(
    name: "lzma",
    path: "ThirdParty/libchdr/deps/lzma-24.05",
    sources: [
      "src/Alloc.c",
      "src/CpuArch.c",
      "src/LzmaDec.c",
    ...
    cSettings: [
      .headerSearchPath("include"),
      // Build LZMA in single-threaded mode to avoid requiring LzFindMt.* and Threads.*
