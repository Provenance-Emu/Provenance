...
class MD_Cart_Type_SBB : public MD_Cart_Type
{
	public:
        ...
        virtual ~MD_Cart_Type_SBB() override;
...
void MD_Cart_Type_SBB::Write16(uint32 A, uint16 V)
{
...
uint8 MD_Cart_Type_SBB::Read8(uint32 A)
{
 if(A < 0x400000)
 {
  if(A >= rom_size)
  {
   MD_DBG(MD_DBG_WARNING, "[MAP_SBB] Unknown read8 from 0x%08x\n", A);
   return(0);
  }
...
uint16 MD_Cart_Type_SBB::Read16(uint32 A)
{
 if(A < 0x400000)
 {
  if(A >= rom_size)
  {
   MD_DBG(MD_DBG_WARNING, "[MAP_SBB] Unknown read16 from 0x%08x\n", A);
   return(0);
  }
