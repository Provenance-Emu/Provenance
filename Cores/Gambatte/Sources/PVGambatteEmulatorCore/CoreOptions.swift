...
extension PVGambatteEmulatorCore: CoreOptional {
...
@objc extension PVGambatteEmulatorCore {
    public func setPalette() {
        if
            let value = PVGambatteEmulatorCore.valueForOption(String.self, "Video.GameBoy (non color) Palette"),
            let index = PVGambatteEmulatorCore.paletteValues.firstIndex(where: { $0.title == value }),
