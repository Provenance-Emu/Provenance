      ...
      const struct retro_system_content_info_override *overrides)
{
   ...
   for (i = 0; overrides[i].extensions; i++)
   {
      ...
      for (j = 0; j < ext_list.size; j++)
      {
         ...
         VLOG(@"[CONTENT OVERRIDE]: File Extension: '%s' - need_fullpath: %s, persistent_data: %s\n",
      ...
      char **error_string)
{
   ...
   VLOG(@"[CONTENT LOAD]: Core requires uncompressed content - "
   ...
   *content_path = temp_path_ptr;
   ...
   VLOG(@"[CONTENT LOAD]: Content successfully extracted to: %s\n",
         temp_path);
      ...
      const struct retro_subsystem_info *special)
{
   ...
   for (i = 0; i < content->size; i++)
   {
      ...
      else
      {
         ...
         else
         {
                ...
                !is_path_accessible_using_standard_io(content_path))
            {
               ...
               VLOG(@"[CONTENT LOAD]: Core does not support VFS"
               ...
               {
                  WLOG(@"[CONTENT LOAD]: Tried copying to cache directory, "
      ...
      {
         VLOG(@"[CONTENT LOAD]: Failed to process content file: %s\n", content_path);
         if (content_data)
      ...
      content_information_ctx_t *content_ctx)
{
   ...
   VLOG(@"[CONTENT LOAD]: Updating firmware status for: %s on %s\n",
         core_info->path,
         ...
         content_ctx->check_firmware_before_loading)
   {
      ...
      VLOG(@"[CONTENT LOAD]: Load content blocked. Reason: %s\n",
            msg_hash_to_str(MSG_FIRMWARE));
...
void content_set_subsystem(unsigned idx)
{
   ...
   VLOG(@"[Subsystem]: Setting current subsystem to: %d(%s) Content amount: %d\n",
...
void content_add_subsystem(const char* path)
{
   ...
   strlcpy(p_content->pending_subsystem_roms[
         ...
         path, pending_size);
   VLOG(@"[Subsystem]: Subsystem id: %d Subsystem ident:"
