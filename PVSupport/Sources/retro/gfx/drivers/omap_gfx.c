...
static const char *omapfb_get_fb_device(void)
{
   ...
   VLOG(@"[video_omap]: Using %s as framebuffer device.\n", fbname);
   return fbname;
}
...
static int omapfb_detect_screen(omapfb_data_t *pdata)
{
   ...
   VLOG(@"[video_omap]: detected %dx%d '%s' (%d) display attached to fb %d and overlay %d.\n",
...
static int omapfb_setup_plane(omapfb_data_t *pdata, int width, int height)
{
   ...
   VLOG(@"omap_video: scaling %dx%d to %dx%d\n", width, height, w, h);
...
static void omap_init_font(omap_video_t *vid, const char *font_path, unsigned font_size)
{
   ...
   {
      VLOG(@"[video_omap]: font init failed\n");
      return;
   }
      ...
      unsigned height, uint64_t frame_count, unsigned pitch, const char *msg)
{
   ...
   {
      VLOG(@"[video_omap]: mode set (resolution changed by core)\n");
