...
public class CloudSyncLogManager {
    ...
    private func setupNotificationObservers() {
        ...
        NotificationCenter.default.publisher(for: iCloudSync.iCloudFileDownloaded)
            .sink { [weak self] notification in
                self?.handleFileDownloaded(notification: notification)
            }
            .store(in: &subscribers)

        NotificationCenter.default.publisher(for: iCloudSync.iCloudFileUploaded)
            .sink { [weak self] notification in
                self?.handleFileUploaded(notification: notification)
            }
            .store(in: &subscribers)

        NotificationCenter.default.publisher(for: iCloudSync.iCloudFileDeleted)
            .sink { [weak self] notification in
                self?.handleFileDeleted(notification: notification)
            }
            .store(in: &subscribers)

        NotificationCenter.default.publisher(for: iCloudSync.iCloudFileRecoveryError)
            .sink { [weak self] notification in
