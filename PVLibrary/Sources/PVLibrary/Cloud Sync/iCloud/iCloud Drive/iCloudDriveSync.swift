...
public enum iCloudDriveSync {
        ...
        await withTaskGroup(of: Void.self) { group in
            group.addTask {
                ...
                if DeltaSkinSyncSupport.isEnabled {
                    sharedDirectories.append(DeltaSkinSyncSupport.directoryName)
                }
                await moveLocalFilesToCloudContainer(directories: sharedDirectories, parentContainer: documentsDirectory)
            }
            group.addTask {
    ...
    static func turnOn() async {
        ...
        if syncMode.isICloudDrive {
            ...
            var nonDatabaseDirectories: Set<String> = [
                "BIOS",
                "Battery States",
                "Screenshots"
//                "RetroArch"
            ...
            if DeltaSkinSyncSupport.isEnabled {
                nonDatabaseDirectories.insert(DeltaSkinSyncSupport.directoryName)
            }
            var nonDatabaseFileSyncer: iCloudContainerSyncer! =
                .init(directories: nonDatabaseDirectories,
...
//            "RetroArch",
        ...
        if DeltaSkinSyncSupport.isEnabled {
            directories.insert(DeltaSkinSyncSupport.directoryName, at: 3)
        }
                        ...
                        do {
                            ...
                            if FileManager.default.fileExists(atPath: fileURL.path, isDirectory: &isDirectory), !isDirectory.boolValue {
                                if directory == DeltaSkinSyncSupport.directoryName {
                                    guard DeltaSkinSyncSupport.isEnabled else {
                                        continue
                                    }
    ...
    static func moveFilesFromiCloudToLocal(directory: String, iCloudContainer: URL) async {
        if directory == DeltaSkinSyncSupport.directoryName && !DeltaSkinSyncSupport.isEnabled {
            DLOG("Skipping DeltaSkins directory sync on unsupported platform.")
            return
        }
        ...
        await moveDirectoryContentsRecursively(from: iCloudDirectory, to: localDirectory, rootDirectoryName: directory)
    }
    ...
    private static func moveDirectoryContentsRecursively(from sourceDir: URL, to destDir: URL, rootDirectoryName: String) async {
        let fileManager = FileManager.default
            ...
            for item in items {
                ...
                do {
                    ...
                    if fileManager.fileExists(atPath: item.path, isDirectory: &isDirectory) {
                        ...
                        if rootDirectoryName == DeltaSkinSyncSupport.directoryName {
                            guard DeltaSkinSyncSupport.isEnabled else {
                                continue
                            }
                            if isDirectory.boolValue {
                                DLOG("Skipping DeltaSkins subdirectory: \(itemName)")
                                continue
                            }
                        ...
                        if isDirectory.boolValue {
                            ...
                            await moveDirectoryContentsRecursively(from: item, to: destItem, rootDirectoryName: rootDirectoryName)
