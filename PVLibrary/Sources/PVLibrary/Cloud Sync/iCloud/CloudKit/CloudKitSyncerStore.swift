...
public class CloudKitSyncerStore {
    ...
    /// Fetch and apply remote changes for ROM metadata and play sessions (CloudKit only).
    public func refreshRomRemoteChanges() async {
        for syncer in romSyncers {
            guard let ck = syncer as? CloudKitRomsSyncer else { continue }
            await ck.fetchAndApplyRemoteChanges()
        }
    }
