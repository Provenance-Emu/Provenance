    ...
    public func determineSystems(for queueItem: ImportQueueItem) async throws -> [SystemType] {
        ...
        if let results = try openVGDB?.searchDatabase(usingKey: "romHashMD5", value: md5),
           ...
           let system = match.system {
            ILOG("Found an exact match using MD5 - \(system.name)")
            return [system]
        }
            ...
            if let results = try openVGDB?.searchDatabase(usingFilename: cleanedFileName, systemID: system.openvgDatabaseID) {
                ...
                if !results.isEmpty {
                    ...
                    for result in results {
                        if cleanFileName(result.gameTitle.lowercased()) == cleanedFileName {
                            return true
                        }
                    }
                }
...
//        }
        ...
        do {
            if let results = try openVGDB?.searchDatabase(usingFilename: fileName) {
                for result in results {
                    if let system = PVEmulatorConfiguration.system(forDatabaseID: result.systemID) {
                        validSystems.append(system)
                    }
                }
            }
