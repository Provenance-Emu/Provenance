...
//  persistence layer — currently the App Group library index, with SwiftData
//  support possible in future.
//
...
///
/// Implementations may be backed by the App Group library index, SwiftData, or
/// any future store. The ThumbnailProvider holds a reference to this protocol
/// so the backend can be swapped without touching QLThumbnailProvider logic.
protocol ThumbnailArtworkDriver {
    ...
    /// Returns `nil` when the App Group library index is unavailable or no game matches.
    ...
    /// save state file at `saveStatePath`, if one is recorded in the App Group library index.
    /// Returns `nil` when the App Group library index is unavailable or no match is found.
