...
static unsigned int hash_func(unsigned int i)
{
	return ((i >> 12) ^ i) & 0x0fff;
}

static unsigned int fast_read(void const *src, unsigned int bytes)
{
#ifndef X86X64
...
static void fast_write(unsigned int f, void *dst, unsigned int bytes)
{
#ifndef X86X64
...
static void memcpy_up(unsigned char *dst, const unsigned char *src, unsigned int n)
{
...
static unsigned int fast_read_safe(void const *src, unsigned int bytes, const unsigned char *invalid)
{
#ifdef memory_safe 
