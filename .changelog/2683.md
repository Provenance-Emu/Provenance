...
- **`supportsExternalDisplay` Core Flag** — Added `supportsExternalDisplay: Bool` property to `PVEmulatorCore` (defaults to `!skipLayout` so standard Metal cores opt in automatically; cores with custom rendering that do not set `skipLayout = true` must explicitly return `false`).
