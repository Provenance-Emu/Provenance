...
- **tvOS Siri Remote mouse pan** — `mouseMoved(atPoint:)` now detects already-relative deltas (values outside `[0,1]`) and forwards them directly, matching the `st_ra_update_mouse_rel()` logic; previously the delta was clamped to `[0,1]` and then differenced, producing incorrect movement
