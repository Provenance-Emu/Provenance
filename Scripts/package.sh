...
usage() {
	...
	echo "  inputXcarchive is path to Provenance.xcarchive"
	echo "  outputPath is path to an EMPTY output directory for Provenance.ipa or Provenance.deb"
...
deb | ipa | ipa-hv | ipa-signed )
	...
	INPUT_APP="$INPUT/Products/Applications/Provenance.app"
	;;
ipa-se )
	NAME="Provenance Lite"
	BUNDLE_ID="org.provenance-emu.provenance-lite"
	INPUT_APP="$INPUT/Products/Applications/Provenance Lite.app"
	;;
* )
	usage
	;;
...
create_deb() {
	local INPUT=$1
	local INPUT_APP="$INPUT/Products/Applications/Provenance.app"
	local OUTPUT=$2
	local FAKEENT=$3
	local DEB_TMP="$OUTPUT/deb"
	local IPA_PATH="$DEB_TMP/Library/Caches/org.provenance-emu.provenance"
...
Package: org.provenance-emu.provenance
Version: ${VERSION}
Section: Productivity
Architecture: iphoneos-arm
Depends: firmware (>=13.0), firmware-sbin, net.angelxwind.appsyncunified
Installed-Size: ${SIZE_KIB}
Maintainer: Joe Mattiello <joe@provenance-emu.com>
Description: Mutli-console emulator for iOS & tvOS
Homepage: https://provenance-emu.com/
Name: Provenance
Author: Joe Mattiello
Depiction: https://cydia.proveance-emu.app/depiction/web/org.provenance-emu.provenance.html
Icon: https://cydia.proveance-emu.app/assets/org.provenance-emu.provenance/icon.png
Moderndepiction: https://cydia.proveance-emu.app/depiction/native/org.provenance-emu.provenance.json
Sileodepiction: https://cydia.proveance-emu.app/depiction/native/org.provenance-emu.provenance.json
...
EOL
	...
	mkdir -p "$IPA_PATH"
	create_fake_ipa "Provenance" "org.provenance-emu.provenance" "$INPUT" "$IPA_PATH" "$FAKEENT"
	dpkg-deb -b -Zgzip -z9 "$DEB_TMP" "$OUTPUT/Provenance.deb"
	rm -r "$DEB_TMP"
}
...
create_fake_ipa() {
	...
	pwd="$(pwd)"
	mkdir -p "$OUTPUT"
	rm -rf "$OUTPUT/Applications" "$OUTPUT/Payload" "$OUTPUT/Provenance.ipa"
