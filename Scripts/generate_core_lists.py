...
def generate_xcfilelist(
    ...
    for core in cores:
        if platform == "ios":
            fname = core.ios_filename()
        else:
            fname = core.tvos_filename()

        # Omit globally-disabled cores from xcfilelists entirely
        if not core.enabled:
            continue
...
def cmd_bootstrap(manifest_path: str) -> None:
    ...
    def register(filename: str, platform: str, enabled: bool, appstore: bool) -> None:
        ...
        if platform == "ios":
            ...
            entry["tvos"] = True
        if not enabled:
            # If disabled in any file, mark globally disabled (best guess)
            pass
        if not appstore:
            entry["appstore"] = False
        if custom and not entry["filename"]:
    ...
    for name, entry in all_filenames.items():
        ios_fname = f"{name}_libretro_ios.dylib" if not entry["filename"] else entry["filename"]
        tvos_fname = f"{name}_libretro_tvos.dylib" if not entry["filename"] else entry["filename"]
