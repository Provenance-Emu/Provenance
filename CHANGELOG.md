...
### Fixed
- **CloudKit/GameImporter Pause During Gameplay** — Fixed regressions where `CloudKitRomsSyncer`, `GameImporter`, and `DirectoryWatcher` continued running I/O work during active gameplay. `setupSaveStateObserver` now skips enqueueing uploads while emulation is active; `performMetadataBootstrap` exits early when paused; `pauseForEmulation` now also suspends syncer-owned `workQueue`s; and `PVEmulatorViewController` calls `pauseForEmulation`/`resumeFromEmulation` directly as a safety net in addition to the `SceneCoordinator` path. (#3014)
