...
- **`dynamicLibretroScanner` feature flag** — Added `PVFeature.dynamicLibretroScanner` to `PVFeatureFlags` (non-AppStore builds only, disabled by default). Enables zero-config testing of buildbot libretro dylibs placed in `Frameworks/` without modifying static plists. (#3080)
