...
typedef struct coreaudio_microphone_handle
{
    AudioComponentInstance audio_unit; /// CoreAudio audio unit
    fifo_buffer_t *sample_buffer;      /// Queue for storing samples
    AudioStreamBasicDescription format; /// Audio format
    bool is_running;                   /// Whether audio unit is running
    bool use_float;                    /// Whether using float samples
} coreaudio_microphone_handle_t;
...
{
    coreaudio_microphone_handle_t *microphone = (coreaudio_microphone_handle_t*)inRefCon;
    AudioBufferList bufferList;
    OSStatus status;

    bufferList.mNumberBuffers = 1;
    bufferList.mBuffers[0].mDataByteSize = inNumberFrames * microphone->format.mBytesPerFrame;
    ...
    if (status == noErr) {
        ...
        RARCH_ERR("[CoreAudio]: Failed to write samples to buffer: %d\n", status);
    }
...
{
    coreaudio_microphone_t *coreaudio = (coreaudio_microphone_t*)driver_context;
    if (coreaudio)
        free(coreaudio);
}
...
{
    coreaudio_microphone_t *coreaudio = (coreaudio_microphone_t*)driver_context;
    coreaudio_microphone_handle_t *microphone = (coreaudio_microphone_handle_t*)microphone_context;
    size_t avail, read_amt;

    if (!coreaudio || !microphone || !buf) {
...
static void *coreaudio_microphone_open_mic(void *driver_context,
                                         ...
                                         unsigned *new_rate)
{
    coreaudio_microphone_handle_t *microphone = (coreaudio_microphone_handle_t*)calloc(1, sizeof(*microphone));
    if (!microphone)
        return NULL;
    ...
    // Set format
    microphone->format.mSampleRate = rate;
    microphone->format.mFormatID = kAudioFormatLinearPCM;
    microphone->format.mFormatFlags = kAudioFormatFlagIsSignedInteger | kAudioFormatFlagIsPacked;
    microphone->format.mFramesPerPacket = 1;
    microphone->format.mChannelsPerFrame = 1;
    ...
    // Create sample buffer
    microphone->sample_buffer = fifo_new(latency * rate * microphone->format.mBytesPerFrame / 1000);
    if (!microphone->sample_buffer) {
        RARCH_ERR("[CoreAudio]: Failed to create sample buffer\n");
        goto error;
    }
